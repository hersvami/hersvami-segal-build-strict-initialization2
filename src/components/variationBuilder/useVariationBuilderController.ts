...
import type { ParametricItem, Project, ProjectBaseline, QuoteScope, Variation } from '../../types/domain';
...
export function useVariationBuilderController(args: Args) {
  const savedDraft = loadBuilderDraft(args.project.id, args.documentType);
  const draftBaseline: ProjectBaseline = savedDraft?.baseline ? { ...getDefaultBaseline(), ...savedDraft.baseline } : getDefaultBaseline();
  ...
  const [ohPct, setOhPct] = useState(args.companyOH);
  const [profitPct, setProfitPct] = useState(args.companyProfit);
  ...
  useEffect(() => restoreKeyNotice(geminiKey, setKeyRestored), []);
  useEffect(() => persistGeminiKey(geminiKey), [geminiKey]);
  ...
  const handleSave = () => { clearBuilderDraft(args.project.id, args.documentType); args.onSave(buildVariationPayload({ ...args, scopes, pricing, varRefQuote, varReason, baseline, tradeAnalyses, analysisFallback, contingencyPct })); };

  return { step, setStep, baseline, baselineAdj, canNext, handleSave, stepProps: { step, documentType: args.documentType, baseline, rawTradeCost, scopeInput, setScopeInput, selectedCategoryId, setSelectedCategoryId, tradeAnalyses, isAnalysing, analysisFallback, showCategoryBrowser, setShowCategoryBrowser, groupedCategories, scopes, geminiKey, setGeminiKey, apiPolishing, varRefQuote, setVarRefQuote, varReason, setVarReason, approvedQuotes, lastAiModel, recogniseFeedback, keyRestored, pricing, ohPct, setOhPct, profitPct, setProfitPct, contingencyPct, setContingencyPct, setBaseline, setScopes, onScopeChange, onRecognise, onPolish, onAddScope, onAddAll, onAcceptSuggestion } };
}
...
function promoteSuggestedItem(analyses: TradeAnalysis[], categoryId: string, unitId: string): TradeAnalysis[] {
  return analyses.map((trade) => {
    ...
    return {
      ...trade,
      items,
      suggestions: trade.suggestions.filter((item) => item.unitId !== unitId),
...
function addSuggestedItemToScope(scope: QuoteScope, suggestion: TradeAnalysis['suggestions'][number], baseline: ProjectBaseline): QuoteScope {
  ...
  const nextItem: ParametricItem = {
    id: generateId(),
    unitId: suggestion.unitId,
    label: suggestion.label,
    unit: suggestion.unit as ParametricItem['unit'],
...
async function recognise(input: string, area: number, key: string, setBusy: (v: boolean) => void, setTrades: (v: TradeAnalysis[]) => void, setFallback: (v: boolean) => void, setModel: (v: string) => void, flash: (v: string) => void) {
  ...
  setTrades(result.trades); setFallback(result.fallback); if (result.model) setModel(result.model);
...
async function polish(input: string, key: string, setBusy: (v: boolean) => void, setInput: (v: string) => void, setModel: (v: string) => void) {
  ...
  setBusy(true); const result = await polishScopeWithAI(input, key); setInput(result.text); if (result.model) setModel(result.model); setBusy(false);
}

function restoreKeyNotice(geminiKey: string, setKeyRestored: (v: boolean) => void) { try { const stored = window.localStorage.getItem(GEMINI_KEY_STORAGE); if (stored && stored === geminiKey) { setKeyRestored(true); setTimeout(() => setKeyRestored(false), 4000); } } catch {} }
function persistGeminiKey(geminiKey: string) { try { if (geminiKey) window.localStorage.setItem(GEMINI_KEY_STORAGE, geminiKey); else window.localStorage.removeItem(GEMINI_KEY_STORAGE); } catch {} }