...
export function StageListEditor({ stages, onChange }: Props) {
  ...
  const updateStage = (index: number, patch: Partial<JobStage>) => {
    const next = [...stages];
  ...
  const handleAddStage = () => {
    ...
    onChange([...stages, { name: draft.name.trim(), trade: draft.trade.trim() || 'General', cost: draft.cost, duration: draft.duration, description: draft.name.trim(), status: 'not-started' }]);
    ...
    <div className="space-y-1">
      ...
      {stages.map((stage, index) => (
        <div key={index} className="group flex items-center gap-2 rounded-lg bg-slate-50 p-2 text-sm">
          ...
          <button onClick={() => onChange(stages.filter((_, i) => i !== index))} className="rounded p-1 text-slate-300 opacity-0 transition-all hover:bg-red-50 hover:text-red-600 group-hover:opacity-100" aria-label="Remove stage"><Trash2 className="h-3.5 w-3.5" /></button>
        </div>
      ))}
      {showAddStage ? <StageDraft draft={draft} setDraft={setDraft} onAdd={handleAddStage} onCancel={() => setShowAddStage(false)} /> : <button onClick={() => setShowAddStage(true)} className="mt-2 flex items-center gap-1 text-sm font-medium text-blue-600 hover:text-blue-700"><Plus className="h-3.5 w-3.5" /> Add Stage</button>}
    </div>
  );
}

function StageDraft({ draft, setDraft, onAdd, onCancel }: { draft: { name: string; trade: string; cost: number; duration: number }; setDraft: (next: { name: string; trade: string; cost: number; duration: number }) => void; onAdd: () => void; onCancel: () => void }) {
  return (
    <div className="mt-2 space-y-2 rounded-lg border border-blue-200 bg-blue-50 p-3">
      <div className="grid grid-cols-2 gap-2"><input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Stage name" className="rounded border border-slate-300 px-2 py-1.5 text-sm" autoFocus /><input value={draft.trade} onChange={(e) => setDraft({ ...draft, trade: e.target.value })} placeholder="Trade (e.g. Plumbing)" className="rounded border border-slate-300 px-2 py-1.5 text-sm" /></div>
      ...
      <div className="flex gap-2"><button onClick={onAdd} disabled={!draft.name.trim()} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-50"><Plus className="h-3 w-3" /> Add Stage</button><button onClick={onCancel} className="rounded-lg px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100">Cancel</button></div>
    </div>
  );
}