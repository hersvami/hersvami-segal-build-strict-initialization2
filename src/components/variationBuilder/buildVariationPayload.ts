import type { Project, ProjectBaseline, QuotePricing, QuoteScope, Variation } from '../../types/domain';
import type { TradeAnalysis } from '../../utils/ai/tradeAnalyser';
import { getEstimatorReviewFlags } from '../../utils/estimatorReview';
import { generateId } from '../../utils/helpers';

type Args = { project: Project; documentType: 'quote' | 'variation'; scopes: QuoteScope[]; pricing: QuotePricing; existingQuotes: Variation[]; varRefQuote: string; varReason: string; baseline: ProjectBaseline; tradeAnalyses?: TradeAnalysis[]; analysisFallback?: boolean; contingencyPct?: number };
...
export function buildVariationPayload(args: Args): Variation {
  ...
  const reviewFlags = getEstimatorReviewFlags(args.scopes, args.pricing, {
    tradeAnalyses: args.tradeAnalyses || [],
    analysisFallback: args.analysisFallback,
    contingencyPct: args.contingencyPct,
  });
  return {
    id: generateId(),
