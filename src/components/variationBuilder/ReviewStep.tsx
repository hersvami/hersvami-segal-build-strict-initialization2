...
export function ReviewStep({ scopes, pricing, ohPct, profitPct, contingencyPct, analysisFallback, tradeAnalyses = [] }: Props) {
  const reviewFlags = getEstimatorReviewFlags(scopes, pricing, {
    analysisFallback,
    tradeAnalyses,
    contingencyPct,
  });
  ...
  if (flags.length === 0) {
    return (
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
        Estimator review clear. No pricing, quantity or scope warnings detected.
      </div>
    );
  }

  const severityClass: Record<EstimatorReviewFlag['severity'], string> = {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <div className="mb-3 flex items-center justify-between">
        <h4 className="text-sm font-semibold text-slate-900">Estimator Review Flags</h4>
      ...
      <div className="space-y-2">
        {flags.map((flag) => (
          <div key={flag.id} className={`rounded-lg border px-3 py-2 text-sm ${severityClass[flag.severity]}`}>
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-medium">{flag.title}</p>
                <p className="mt-0.5 text-xs opacity-90">{flag.message}</p>
              </div>
              {flag.categoryLabel && <span className="shrink-0 text-xs opacity-75">{flag.categoryLabel}</span>}
            </div>
          </div>
        ))}
      </div>
